...
@RestController
@RequestMapping("/ocBug")
public class OCBugController {

    private final UserService userService;
    private final DesignerBugService designerBugService;

    public OCBugController(UserService userService, DesignerBugService designerBugService) {
    ...
    public PageResult<DesignerBug> query(@RequestBody BugQueryParams params) {
        return designerBugService.query(params, BugType.OC);
    }

    @PostMapping("create")
    public Result<String> createBug(@RequestBody BugCreateDto createDto){
        createDto.setType(BugType.OC);
